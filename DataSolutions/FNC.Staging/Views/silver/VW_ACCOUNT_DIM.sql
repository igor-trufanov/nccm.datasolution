...
AS
    SELECT 
        UPPER(CONCAT(stm.GLACCTGRP_KEY, '||', stm.GLACCTGRP_NAME, '||', stm.RECORD_NUMBER, '||', stm.ACCOUNT_NUMBER)) AS UQ_KEY,
        HASHBYTES('MD5', UPPER(CONCAT(stm.GLACCTGRP_KEY, '||', stm.GLACCTGRP_NAME, '||', stm.RECORD_NUMBER, '||', stm.ACCOUNT_NUMBER))) AS UQ_KEY_HASH,

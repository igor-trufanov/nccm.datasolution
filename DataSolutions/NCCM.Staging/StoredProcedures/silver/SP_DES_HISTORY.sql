CREATE PROCEDURE [silver.NCCM].SP_DES_HISTORY
AS
BEGIN
    ...
    BEGIN TRANSACTION TR1;
        ...
        ;WITH trg AS (
            SELECT * FROM [silver.NCCM].DES_HISTORY WHERE ROW_IS_CURRENT = 1
        )
        MERGE trg
        USING [silver.NCCM].VW_DES_HISTORY AS src
        ON (
        ...
        USING (
            SELECT
                *
            FROM [silver.NCCM].VW_DES_HISTORY
        ) AS src
        ON (
            trg.UQ_KEY_HASH = src.UQ_KEY_HASH

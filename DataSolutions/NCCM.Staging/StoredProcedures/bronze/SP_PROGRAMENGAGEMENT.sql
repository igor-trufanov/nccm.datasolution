...
BEGIN
    ...
    WHEN NOT MATCHED BY TARGET THEN 
        ...
        WITH cte AS (
            SELECT 
                tbl.LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS,
                tbl.PMDM__CONTACT__C,
                ROW_NUMBER() OVER(PARTITION BY tbl.PMDM__CONTACT__C ORDER BY tbl.CREATEDDATE DESC, ID ASC) AS LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS_CALC
            FROM [bronze.NCCM].PROGRAMENGAGEMENT AS tbl
            WHERE tbl.PMDM__CONTACT__C IS NOT NULL
        )
        UPDATE cte
            SET LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS = LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS_CALC
        WHERE 
            LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS IS NULL 
            OR LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS_CALC <> LAST_CONTACT_ACROSS_ALL_THEIR_ENGAGEMENTS;

    UPDATE trg
            SET trg.ISDELETED = 1
        FROM [bronze.NCCM].PROGRAMENGAGEMENT AS trg
            LEFT JOIN [bronze.NCCM].VW_PMDM_PROGRAMENGAGEMENT AS src
                ON trg.ID = src.ID
        WHERE 
            trg.ISDELETED = 0 
            AND src.ID IS NULL;
